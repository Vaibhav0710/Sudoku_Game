...
class GameGenerator {
     ...
     * 2. For each value in the range 1..9, allocate that value 9 times based on the
     * following constraints:
     * - A Random coordinate on the grid is selected. If it is empty, a Random value
     * is allocated.
     * - The resulting allocation must not produce invalid rows, columns, or
     * squares.
     ...
     * (and thus proven to be solvable),
     * and randomly assign a certain number of tiles to be equal to 0. It appears
     * that there is no straight
     * forward way to check if a puzzle is still solvable after removing the tiles,
     * beyond using another algorithm
